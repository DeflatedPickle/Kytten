...
try:
    ...
    try:
        ...
        print(sys.stderr,
              "Warning: using 'safe_eval' to process json files, please upgrade to Python 2.6 or install simplejson")
        import safe_eval
...
class TextureGraphicElement:
    ...
    @staticmethod
    def get_content_size(width, height):
        return width, height

    @staticmethod
    def get_needed_size(content_width, content_height):
...
class UndefinedGraphicElement(TextureGraphicElement):
    def __init__(self, theme, color, batch, group, texture):
        super().__init__(theme, texture, color, batch, group)
...
class Theme(ScopedDict):
    ...
    def __init__(self, arg, override={}, default=DEFAULT_THEME_SETTINGS,
        ...
        @param default Initial dictionary entries before handling input_
        ...
        self.textures = {}
        self._update_with_images(self, input_)
        self.update(override)
    ...
    def _get_texture(self, filename):
        ...
        """
        if not self.textures.get(filename):
    ...
    def _update_with_images(self, target, input__):
        """
        Update a ScopedDict with the input_ dictionary.  Translate
        images into texture templates.

        @param target The ScopedDict which is to be populated
        @param input__ The input_ dictionary
        """
        for k, v in input__.items():
